-- Comprehensive FMCG Market Schema with RLS Policies (TBWA Client Identifiers)
...
DROP POLICY IF EXISTS "Allow service role full access to brands" ON public.brands;
CREATE POLICY "Allow service role full access to brands" ON public.brands TO service_role USING (true);

-- Generic policies for other tables (authenticated users can read/write, service_role has full access)
DO $$
DECLARE
    tbl_name text;
BEGIN
    FOR tbl_name IN
        ...
        AND table_name NOT IN ('brands', 'schema_migrations', 'users') -- Exclude brands and internal Supabase tables
    LOOP
        EXECUTE FORMAT('DROP POLICY IF EXISTS "Enable read access for all authenticated users on %I" ON public.%I;', tbl_name, tbl_name);
        EXECUTE FORMAT('CREATE POLICY "Enable read access for all authenticated users on %I" ON public.%I FOR SELECT USING (auth.role() = ''authenticated'');', tbl_name, tbl_name);

        EXECUTE FORMAT('DROP POLICY IF EXISTS "Enable insert for authenticated users on %I" ON public.%I;', tbl_name, tbl_name);
        EXECUTE FORMAT('CREATE POLICY "Enable insert for authenticated users on %I" ON public.%I FOR INSERT WITH CHECK (auth.role() = ''authenticated'');', tbl_name, tbl_name);

        EXECUTE FORMAT('DROP POLICY IF EXISTS "Enable update for authenticated users on %I" ON public.%I;', tbl_name, tbl_name);
        EXECUTE FORMAT('CREATE POLICY "Enable update for authenticated users on %I" ON public.%I FOR UPDATE USING (auth.role() = ''authenticated'');', tbl_name, tbl_name);

        EXECUTE FORMAT('DROP POLICY IF EXISTS "Enable delete for authenticated users on %I" ON public.%I;', tbl_name, tbl_name);
        EXECUTE FORMAT('CREATE POLICY "Enable delete for authenticated users on %I" ON public.%I FOR DELETE USING (auth.role() = ''authenticated'');', tbl_name, tbl_name);

        -- Policy for service_role to ensure it bypasses RLS (although by default service_role bypasses RLS, explicit policy can be good for clarity/edge cases)
        EXECUTE FORMAT('DROP POLICY IF EXISTS "Allow service role full access on %I" ON public.%I;', tbl_name, tbl_name);
        EXECUTE FORMAT('CREATE POLICY "Allow service role full access on %I" ON public.%I TO service_role USING (true) WITH CHECK (true);', tbl_name, tbl_name);
